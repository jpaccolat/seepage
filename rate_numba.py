...
def q_approx(stage: float, dis_WT:float, cl_cond: float, cl_th: float,
    ...
    if aq_para == 'vGM':
        q = q_approx_vGM(stage, dis_WT, cl_cond, cl_th, aq_cond, aq_scale,
                               aq_shape)
    elif aq_para == 'BCB':
        q = q_approx_BCB(stage, dis_WT, cl_cond, cl_th, aq_cond, aq_scale,
                               aq_shape)
...
def q0_approx_full_vGM(cl_cond, cl_th, aq_cond, aq_scale, aq_shape):
    ...
    q0_ns = aq_cond * (1 + x**a_ns)**(-xi/a_ns)
    
    a_sh = C_SH_1 + C_SH_2 * xi
    q0_sh = cl_cond * (1 + (x_sh / x)**a_sh)**(xi / a_sh)
...
def q_approx_vGM(stage: float, dis_WT:float, cl_cond: float, cl_th: float,
                       aq_cond: float, aq_scale: float, aq_shape: float):
    
    q1 = (stage + cl_th + dis_WT) / (cl_th / cl_cond + dis_WT / aq_cond)
    q2 = q_approx_full_vGM(stage, cl_cond, cl_th, aq_cond, aq_scale, aq_shape)
    q = min(q1, q2)
...
@vectorize([float64(float64, float64, float64, float64, float64, float64, float64)])
def q_approx_BCB(stage: float, dis_WT:float, cl_cond: float, cl_th: float,
                       aq_cond: float, aq_scale: float, aq_shape: float):
    
    q1 = (stage + cl_th + dis_WT) / (cl_th / cl_cond + dis_WT / aq_cond)
    q2 = q_approx_full_BCB(stage, cl_cond, cl_th, aq_cond, aq_scale, aq_shape)
    q = min(q1, q2)
